...
class PreActBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = self.shortcut(out) if hasattr(self, 'shortcut') else x #todo diffierent
        out = self.conv1(out)
...
class ReducedResNet(PreActResNet):
    def __init__(self, block, num_blocks, in_channels=3,
                 embed_dim=0, norm=True, num_classes=10): #TODO only support embed_dim=0
        super(PreActResNet, self).__init__()
        self.embed_dim = embed_dim
        self.norm = norm
        self.has_embedding = embed_dim > 0
        self.in_planes = 20
        self.last_planes = 160 * block.expansion

        self.conv1 = conv3x3(in_channels, 20)
        self.stage1 = self._make_layer(block, 20, num_blocks[0], stride=1)
        self.stage2 = self._make_layer(block, 40, num_blocks[1], stride=2)
        self.stage3 = self._make_layer(block, 80, num_blocks[2], stride=2)
        self.stage4 = self._make_layer(block, 160, num_blocks[3], stride=2)
        self.bn_last = nn.BatchNorm2d(self.last_planes)
        # self.num_feature = embed_dim
        if self.has_embedding:
            self.embedding = nn.Linear(self.last_planes, self.embed_dim, bias=False)
        self.last = nn.Linear(self.last_planes, num_classes)
...
class PreActResNetImage(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, in_channels=3):
        super(PreActResNetImage, self).__init__()
        self.in_planes = 64
        last_planes = 512*block.expansion

        self.conv1 = conv7x7(in_channels, 64)
        self.stage1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        self.stage2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        self.stage3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.stage4 = self._make_layer(block, 512, num_blocks[3], stride=2)
...
def ResNet32_cifar_out10(out_dim=10):
    return PreActResNet_cifar(PreActBlock, [5 , 5 , 5 ], [16, 32, 64], num_classes=out_dim)
def ResNet32_cifar_out100(out_dim=100):
    return PreActResNet_cifar(PreActBlock, [5 , 5 , 5 ], [16, 32, 64], num_classes=out_dim)
...
def resnet50(out_dim=10):
    return PreActResNetImage(PreActBottleneck, [3, 4, 6, 3], num_classes=out_dim)
...
def reducedR18(out_dim=10):
    return ReducedResNet(PreActBlock, [2, 2, 2, 2], num_classes=out_dim)
