...
def run(args):
    ...
    # dataset_name = '{}'.format(args.first_split_size) + 'split_' + args.dataset
    # hyperparam = 'reg{}'.format(args.reg_coef) + '_lr{}'.format(args.model_lr) + 'head{}'.format(args.head_lr)
    # save_dir = os.path.join('./checkpoints', os.path.join(dataset_name, hyperparam))
    # if not os.path.exists(save_dir):
    #     os.makedirs(save_dir)
    agent_config = {'model_lr': args.model_lr, 'head_lr': args.head_lr,
                    ...
                    'cls_num': args.first_split_size,'singular': args.singular
                    }
...
def get_args(argv):
    ...
    parser.add_argument('--agent_type', type=str, default='layer_me_cons', help="The type (filename) of agent")
    parser.add_argument('--agent_name', type=str, default='distillewc', help="The class name of agent")
    ...
    parser.add_argument('--head_lr', type=float, default=5e-3, help="Classifier(Head) Learning rate")
    ...
    parser.add_argument('--reg_coef', nargs="+", type=float, default=[1],
    ...
    for reg_coef in reg_coef_list:
        ...
        for r in range(args.repeat):
            ...
            for i in range(len(task_names)):
                ...
                print('Task', train_name, 'Average Accuary:', avg_acc_history[i])
